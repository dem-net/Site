        ...
        <div class="container is-fluild">
            ...
            <div class="card">
                <header class="card-header">
                    <p class="card-header-title">Draw a rectangle using the tool&nbsp;>&nbsp;Choose your options&nbsp;>&nbsp;Generate
                        and see the 3D model.</p>
                </header>
                    ...
                    <div class="content">
                        <section>
                                ...
                                <div class="column">
                                    <b-tabs type="is-toggle" expanded>
                                        <b-tab-item label="3D settings" icon-pack="fa" icon="cube">
                                            ...
                                            <div class="columns">
                                                <div class="column">
                                                    <DatasetSelector :dataSet="this.requestParams.dataSet"
                                                ...
                                                <div class="column">
                                                    ...
                                                    <b-field>
                                                        <b-radio-button v-model="requestParams.format"
                                                                        native-value="glTF">Binary glTF
                                                        </b-radio-button>
                                            ...
                                            <div class="columns">
                                                ...
                                                <div class="column" v-show="showTextureOptions">
                                                    ...
                                                    <ImagerySelector v-show="showTextureOptionsProvider"
                                                                     :provider="requestParams.imageryProvider"
                                                ...
                                                <div class="column">
                                                    <b-field label="Z multiplier">
                                                        <b-slider v-model="requestParams.zFactor" size="is-medium"
                                                                  :min="1" :max="10" :step=".5"></b-slider>
                                                    </b-field>
                                                </div>
                                                <!-- TIN-->
                                                <div class="column" v-show="false">
                                                    <b-field label="Generate TIN"
                                                             v-if="this.requestParams.format == 'glTF'">
                                                        <b-tooltip
                                                                label="Decimates the mesh (reduces number of triangles). This is a long operation, be patient."
                                        ...
                                        <b-tab-item label="OSM settings" icon-pack="fas" icon="building">
                                            ...
                                            <div class="columns">
                                                ...
                                                <div class="column">
                                                    <b-field label="Use OSM buildings colors"
                                                             v-if="requestParams.osmBuildings">
                                                        <b-switch v-model="requestParams.useOsmBuildingsColor">
                                                        </b-switch>
                                                    </b-field>
                                                </div>
                                                <div class="column">
                                                    <b-field label="Custom building color"
                                                             v-if="!requestParams.useOsmBuildingsColor && requestParams.osmBuildings">
                                                        <swatches v-model="requestParams.buildingsColor"
                                                                  colors="text-advanced" popover-to="left"></swatches>
                                                    </b-field>
                                                </div>
                                            </div>
                            ...
                            <div class="buttons is-centered">
                                <!-- Generation -->
                                <b-button @click="generateModel" :disabled="!requestParams.bbox" icon-pack="fas"
                                          icon-left="fas fa-globe-americas">
                                    Generate 3D model
                                </b-button>
                                <!-- Textures -->
                                <b-dropdown hoverable aria-role="list" :disabled="!this.glbFile">
                                    ...
                                    <b-dropdown-item has-link aria-role="menuitem" v-if="this.textureFiles.heightMap">
                                        <a :href="this.textureFiles.heightMap" target="_blank"
                                    ...
                                    <b-dropdown-item has-link aria-role="menuitem" v-if="this.textureFiles.normalMap">
                                        <a :href="this.textureFiles.normalMap" target="_blank"
                                           rel="noopener noreferrer">
                                            <b-icon pack="fas" icon="image"></b-icon>
                                            Normal map
                                        </a>
                                ...
                                <!-- Download -->
                                <b-button icon-pack="fas" icon-left="fas fa-download" :disabled="!this.glbFile" tag="a"
                                          :href="this.glbFile" @click="modelDownload">
                                    Download model
                                </b-button>


                                <!-- Attributions -->
                                <b-button icon-pack="fas" icon-left="fas fa-copyright" :disabled="!this.glbFile" tag="a"
                                          href="#attributions">
                                    Attributions
                                </b-button>
                                ...
                                <a :disabled="!this.glbFile" :href="this.SketchFabLoginUrl" class="button is-default"
                                   target="_blank">
                                    <span class="icon is-small"><img src="../assets/sketchfablogo.png" width="22"
                                                                     height="22" style="align: center"/></span>
                                    <span>SketchFab export...</span>
                                </a>
                            </div>

                            <p>
                                <b-progress v-show="serverProgress" :value="serverProgressPercent" size="is-large"
                                            :type="progressType" show-value>
                            ...
                            <ThreeDviewer
                                    :key="glbFile"
                                    @on-load="onLoad"
                                    v-if="glbFile && (this.requestParams.format == 'glTF' || this.requestParams.format == 'STL')"
                                    :threeDFilePath="glbFile"
                            ...
                            <b-loading :is-full-page="isLoadingFullPage" :active.sync="isLoading"
...
export default {
    ...
    components: {ThreeDviewer, MapRectangle, DatasetSelector, ImagerySelector, Swatches, Attributions},
    mounted() {
